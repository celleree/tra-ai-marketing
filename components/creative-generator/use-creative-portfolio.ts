...
export function useCreativePortfolio() {
  ...
  const [running, setRunning] = useState(false), [pausing, setPausing] = useState(false), [error, setError] = useState('');
  const current = useRef<PortfolioResponse | null>(null), busy = useRef(false), stop = useRef(false), mounted = useRef(true);
  ...
  const execute = async (task: () => Promise<PortfolioResponse>, advance: boolean) => {
    ...
    busy.current = true; stop.current = false; setRunning(true); setPausing(false); setError('');
    try {
      const value = await task(); update(value);
      if (value.error) throw new Error(value.error);
      if (advance) await runPortfolio(value, update, () => stop.current || !mounted.current);
    } catch (failure) {
      if (mounted.current) setError(failure instanceof Error ? failure.message : 'Portfolio work failed. Reload saved progress.');
    } finally {
      busy.current = false;
      if (mounted.current) { setRunning(false); setPausing(false); }
    }
  };
  ...
  const pause = () => { stop.current = true; setPausing(true); };
  return { response, running, pausing, error, start, resume, retry, restoreSaved, pause };
}
