...
const mocks = vi.hoisted(() => ({ prepare: vi.fn(), restore: vi.fn(), render: vi.fn(), list: vi.fn() }));
...
beforeEach(() => {
  records = []; mocks.prepare.mockReset(); mocks.restore.mockReset(); mocks.render.mockReset();
  mocks.list.mockReset().mockImplementation(async () => records);
  mocks.prepare.mockImplementation(async request => ({ ...portfolioSnapshot(newCreativePortfolio(request)),
    videoFrameSet: null, brandLogo: null, reserveLogoArea: false }));
...
describe('bounded resumable portfolio execution', () => {
  it('plans once, then completes 36 single-image steps without double-charging the generation quota', async () => {
