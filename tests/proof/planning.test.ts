...
const { listProofRecordsMock } = vi.hoisted(() => ({
  listProofRecordsMock: vi.fn<() => Promise<ProofRecord[]>>(async () => []),
}));
vi.mock('@/lib/proof/storage', () => ({
  listProofRecords: listProofRecordsMock,
}));
...
const review = (
  hex: string,
  overrides: Partial<ReviewProofRecord> = {}
): ReviewProofRecord => ({
...
const caseStudy = (
  hex: string,
  overrides: Partial<CaseStudyProofRecord> = {}
): CaseStudyProofRecord => ({
  id: `proof_${hex.repeat(32)}`,
  type: 'case-study',
...
const analysis = {
  ...
  visibleText: [],
  visualStructure: 'Headline over graphic',
  hookOrAngle: 'Bank levy concern',
  offerOrCta: 'Talk with TRA',
  styleNotes: 'Calm',
  preserve: ['hierarchy'],
...
const strategy = {
  ...
  persona: 'Taxpayer facing a bank levy',
  painPoint: 'Bank levy concern',
  desiredOutcome: 'Clear next steps',
  emotion: 'Relief',
  hook: 'Understand the next step',
  cta: 'Talk with TRA',
  offer: null,
  soWhat: {
  ...
  execution: {
    taxDocumentReference: 'none',
    subjectSource: 'non-human',
    composition: 'single-focus',
    imageTreatment: 'minimal-graphic',
    textDensity: 'low',
    ctaTreatment: 'button',
...
const concept = (index: number) => ({
  ...
  strategy: {
    ...
    conceptDetails: {
      ...conceptDetails,
      proposition: `Distinct proposition ${index}`,
    },
    soWhat: {
      ...strategy.soWhat,
      surfaceMessage: `Distinct surface ${index}`,
    },
  },
...
describe('Proof planning retrieval', () => {
  it('preserves an approved ACTIVE Review exactly and includes attribution only when allowed', () => {
    const attributed = review('a', {
    ...
    const result = selectProofForPlanning(
      ...
      'Need a bank levy proof concept'
    );
    ...
    expect(result.find((item) => item.id === unattributed.id)).not.toHaveProperty(
      'attribution'
    );
  });
  ...
  it('excludes unapproved, legacy approval-absent and INACTIVE records', () => {
    const unapproved = review('d', { advertisingUseApproved: false });
    const legacy = review('e');
    delete legacy.advertisingUseApproved;
    const inactive = review('f', { status: 'INACTIVE' });

    expect(
      selectProofForPlanning(
        [unapproved, legacy, inactive],
        'Bank levy proof'
      )
    ).toEqual([]);
  });
  ...
  it('keeps relevant retrieval deterministic and bounded', () => {
    const records = '0123456789ab'.split('').map((hex, index) =>
      review(hex, {
    ...
    const second = selectProofForPlanning([...records].reverse(), 'bank levy');

    expect(first).toEqual(second);
    expect(first).toHaveLength(MAX_PLANNING_PROOF_RECORDS);
    expect(
      first.reduce((total, item) => total + JSON.stringify(item).length, 0)
  ...
  it('sends Astra only eligible Proof records and never verifiedFacts', async () => {
    ...
    expect(input.proofCatalog.map((item: { id: string }) => item.id)).toEqual([
      approvedCase.id,
      approvedReview.id,
    ]);
    expect(JSON.stringify(input.proofCatalog)).not.toContain(
      'SOURCE_ONLY_VERIFIED_FACT'
    );
    expect(JSON.stringify(input.proofCatalog)).not.toContain(legacy.id);
    expect(body.input[0].content[0].text).toContain(
      'quote it only verbatim'
    );
    expect(body.input[0].content[0].text).toContain(
      'never broaden it into a universal outcome'
    );
  });
});
