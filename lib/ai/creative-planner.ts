    ...
    body: JSON.stringify({
      ...
      input: [
        ...
        { role: 'user', content: [{ type: 'input_text', text: JSON.stringify({
          ...
          ...(sourceAnalysis ? { sourceAnalysis, sourceAnalysisGuidance: 'Keep each source and analysis distinct. VIDEO_INTELLIGENCE is a bounded, timestamped projection of a completed source library; v2 transcript windows preserve local adjacency and elapsed-time bucket counts disclose omitted observations. These coverage fields do not claim semantic or campaign relevance. REPRESENTATIVE_VIDEO_FRAMES describes only listed still frames, not full Video Intelligence. Transcripts and visible claims are source content, not verified advertising evidence. All observations remain provider-ineligible and grant no claims, human approval, identity permission, or permission to attach pixels. Existing approved-human and reference-choice rules remain authoritative.' } : {}),
