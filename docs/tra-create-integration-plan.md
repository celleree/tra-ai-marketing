...
PR #238 merged after live reviewed HEAD `7413da0abf822d9e8734cf2522091b76ff2ee1c2` and passing required checks were verified. B1.2 adds inactive `stepPortfolioVideoDependency`: discovery checkpoints the existing identity/job reference before a later START, and each subsequent call performs at most one existing bounded job unit. Existing services own child leases, checkpointed progress and finalization. Valid completed artifacts are loaded and bound to the compact dependency; saved completion does not consult current analyzer settings or rehydrate/reanalyze the video. Pending source/settings drift fails explicitly.
