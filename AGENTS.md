...
- Broad orchestration/parallel-work rules -> load `docs/agent-workflow.md` and, only if parallelism is being considered, `docs/parallel-coding.md`.
...
Risk is based on the change's actual effect:
...
- do not treat stale PR-body HEAD/check metadata as live GitHub state.
...
Reasoning: use Low for straightforward/local work, Medium for normal implementation/investigation, High for difficult ambiguity/integration/consequential review, and Extra-high only when clearly justified.
...
Within an approved development scope, proceed without repeated approval for reversible branch-local work such as reading files, editing the feature branch, focused tests/checks, commits/checkpoints, PR updates, and repairs inside scope.

Require explicit approval before destructive/irreversible/production-impacting actions, advertising spend/publishing, credentials/secrets changes, production data/assets mutation, or promotion of `staging` to `main`.

## Durable decisions

Keep canonical docs concise. Record final durable decisions, not raw conversations or debugging history.

Prefer the first effective enforcement point: regression test/eval -> deterministic guard/validation -> reusable helper -> code/config -> existing canonical doc -> GitHub Issue for deferred work. Do not add new agent rules when code/tests or an existing document can enforce the behavior more directly.

Detailed coordination, threat-model proportionality, review freshness, handoff, parallelism, and issue-completion rules live in `docs/agent-workflow.md`.